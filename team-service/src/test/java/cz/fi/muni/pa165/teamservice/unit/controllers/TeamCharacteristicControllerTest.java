...
import cz.fi.muni.pa165.teamservice.api.controllers.TeamCharacteristicController;
...
public class TeamCharacteristicControllerTest {
	...
	void getCharacteristicById() throws Exception {
		...
		mockMvc.perform(get("/v1/team-characteristics/{id}", characteristicId))
	...
	void getCharacteristicById_notFound() throws Exception {
		...
		mockMvc.perform(get("/v1/team-characteristics/{id}", characteristicId)).andExpect(status().isNotFound());
	...
	void updateTeamCharacteristicCharacteristic() throws Exception {
		...
		mockMvc
			.perform(put("/v1/team-characteristics/{id}", characteristicId).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(characteristicDTO)))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.guid").value(characteristicId.toString()));
	...
	void deleteTeamCharacteristicCharacteristic() throws Exception {
		...
		mockMvc.perform(delete("/v1/team-characteristics/{id}", characteristicId)).andExpect(status().isNoContent());
	...
	void getCharacteristicsByTeamId_empty() throws Exception {
		...
		mockMvc.perform(get("/v1/team-characteristics/team/{teamId}", teamId))
			.andExpect(status().isOk())
	...
	void getCharacteristicById_invalidUuid() throws Exception {
		mockMvc.perform(get("/v1/team-characteristics/{id}", "invalid-uuid")).andExpect(status().isBadRequest());
	...
	void getCharacteristicsByTeamId_invalidUuid() throws Exception {
		mockMvc.perform(get("/v1/team-characteristics/team/{teamId}", "invalid-uuid"))
			.andExpect(status().isBadRequest());
